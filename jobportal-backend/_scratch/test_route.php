...
// Test if we can resolve the controller using the class name from the use statement
try {
    $controller = app()->make(RecommendationController::class);
    echo "Controller resolved using use statement successfully: " . get_class($controller) . "\n";
} catch (Exception $e) {
    echo "Error resolving controller using use statement: " . $e->getMessage() . "\n";
}