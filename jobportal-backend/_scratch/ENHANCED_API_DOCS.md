...
The job portal API serves as the backend interface for a platform that connects job seekers with employers. It provides functionality for user authentication, job listings, applications, company profiles, and AI-powered job recommendations.

## User Roles

The application has three main user roles:
1. **Job Seekers** - Users looking for employment opportunities
2. **Employers** - Users posting job listings and managing their company profiles
3. **Admins** - Users with special privileges to verify companies and manage the platform
...
- **Purpose**: Sends a new email verification link to user's email
- **Functionality**: Generates verification token and sends email
- **User Access**: Authenticated users (job seekers, employers)
- **Response**: Success message
- **How It Works**:
  1. Identifies user from Authorization token
...
- **Purpose**: Verifies a user's email address using verification token
- **Functionality**: Updates user's email_verified_at timestamp
- **User Access**: Users with valid verification token
- **Response**: Success message and redirect URL
- **How It Works**:
  1. Validates user ID and hash combination
...
- **Functionality**: Returns detailed user profile data
- **User Access**: Authenticated users (job seekers, employers)
- **Response**: User profile object
- **How It Works**:
  1. Identifies user from Authorization token
  2. Retrieves user profile data from database
  3. Returns complete profile information

#### Update User Profile
- **Endpoint**: `PUT /api/profile`
- **Purpose**: Updates the authenticated user's profile information
- **Functionality**: Validates and updates user profile data
- **User Access**: Authenticated users (job seekers, employers)
- **Request Body**:
